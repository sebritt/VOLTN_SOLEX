...
def read_serial(port):
    ...
    regex = re.compile(
        r"Paquet reçu \(\d+ octets\) : ([\d\.-]+),([\d\.-]+),([\d\.]+)km/h,([\d\.]+)m,(\d+:\d+:\d+) T:(\d+)C Hdg:(\d+)"
    )
    ...
    while True:
        try:
            ...
            heure = match.group(5)
            temp = int(match.group(6))
            heading = int(match.group(7))
            ...
            formatted = (
                ...
                f"Vitesse         : {speed:.1f} km/h\n"
                f"Heure GPS       : {heure}\n"
                f"Température     : {temp} °C\n"
                f"Orientation     : {heading}°\n\n"
                f"Vitesse max     : {vmax:.1f} km/h\n"
                f"Vitesse moyenne : {vmean:.1f} km/h"
            )
