...
void setup() {
  ...
  delay(1000);
  Serial.begin(115200);   //Initialisation port serie
  while (!Serial) {;}
  delay(1000);
  lcd.clear();
  lcd.print("INIT SERIAL DONE");
...
void loop() {
  ...
  map_gaz=map(raw_gaz,EEPROM_min_gaz,EEPROM_max_gaz,100,255);
  if(map_gaz<0)
      {
        map_gaz=0;
      }
  ...
  if ( UART.getVescValues() ) {
    lcd.setCursor(5,1);
    lcd.print((int)UART.data.avgMotorCurrent);
