...
#define MAX_VESC_SAVE 255
#define MIN_VESC_SAVE 0
...
void loop() {
  ...
  if ( vesc.getVescValues() ) {
    lcd.setCursor(0,1);
    lcd.print((int)vesc.data.avgMotorCurrent);
    lcd.print("A");
    if((int)vesc.data.avgMotorCurrent<10)
    {
      lcd.print(" ");    
    }
  }
...
void write_gaz()
{
  vesc.setCurrent(map_gaz);
  /*
  if(digitalRead(SW4) && HS)
  {
  VESC.writeMicroseconds(map_gaz);
  }
  else
  {
      VESC.writeMicroseconds(MIN_VESC_SAVE);
      lcd.setCursor(8,1); 
      lcd.print("HS ");
      HS=0;
  }
